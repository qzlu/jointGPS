...
<template>
    <section>
        ...
        <!--<lock-select-dialog v-on:child="get"></lock-select-dialog>-->
        <!-- 选择设备1-->
        <!--  <div class='sma'>
        ...
        <div class="mymodal">
            ...
            <div class="mapModal">
                ...
                <div style="padding: 15px">
                    <p>
                        <span>_时间_：</span>{{FStartDateTime}}</p>
                    <p>
                        <span>_箱门开关类型_：</span>{{FDataType}}</p>
                    <p>
        ...
        <div id="operate_wrapper">
            <div class="box-header">
                ...
                <div class="block">
                    <span class="operatExp">_选择车辆_/_箱号_</span>
                    <div class="search-input-wrap" data-toggle="modal" data-target="#modal">
                        <!---->
                        <a href="javascript:;" class="get-carList-dialog">
            ...
            <el-tab-pane label="_汇总_" name="all">
                <div class="el_table_wrap" id="el_table_wrap">
                    <el-table :data="tableData3" stripe style="width: 100%" :height="table_height" ref="multipleTable" @selection-change="handleSelectionChange"
                        ...
                        <el-table-column prop="FShortName" label="_公司简称_" width="140" show-overflow-tooltip>
                        </el-table-column>
                        <el-table-column prop="FVehicleName" label="_箱号_" width="140" show-overflow-tooltip>
                        </el-table-column>
                        <el-table-column prop="FCount" label="_开关门次数_" width="180" sortable show-overflow-tooltip></el-table-column>
                        ...
                        <el-table-column prop=" " label=" " min-width="800"></el-table-column>
                    </el-table>
                </div>
            ...
            <el-tab-pane label="_全部_" name="first">
                <div class="el_table_wrap">
                    <el-table :data="tableData4" :height="table_height" style="width: 100%;" ref="multipleTable" @selection-change="handleSelectionChange"
                        ...
                        <el-table-column prop="FVehicleName" label="_集装箱号_" width="140" show-overflow-tooltip>
                        </el-table-column>
                        <el-table-column prop='FStartDateTime' label="_时间_" width="180" sortable></el-table-column>
                        <el-table-column prop="FStartMileage" label="_里程_" width="120" sortable></el-table-column>
                        <el-table-column prop="FDataType" label="_箱门状态_" width="120"></el-table-column>
                        <el-table-column prop="sposition" label="_位置_" width="260" show-overflow-tooltip></el-table-column>
                        ...
                        <el-table-column prop=" " label=" " min-width="760"></el-table-column>
                    </el-table>
                </div>
                <div class="pagination">
                    <div class="block">
            ...
            <el-tab-pane v-for="(item, index) in editableTabs" :key="item.name" :label="item.title" :name="item.name" closable>
                <div class="el_table_wrap">
                    <el-table :data="tableData5" :height="table_height" style="width: 100%;" ref="multipleTable" @selection-change="handleSelectionChange"
                        ...
                        <el-table-column prop='FStartDateTime' label="_时间_" width="180"></el-table-column>
                        <el-table-column prop="FStartMileage" label="_里程_" width="120"></el-table-column>
                        <el-table-column prop="FDataType" label="_箱门状态_" width="120" sortable></el-table-column>
                        <el-table-column prop="sposition" label="_位置_" width="260" show-overflow-tooltip></el-table-column>
                        <!--<el-table-column prop="FParkingEndDateTime" label="_结束停车时间_" width="200" sortable></el-table-column>
                        <el-table-column prop="position" label="_位置_" width="250" show-overflow-tooltip align='left'></el-table-column>-->
                        <el-table-column prop=" " label=" " min-width="200"></el-table-column>
                    </el-table>
                </div>
                <div class="pagination">
                    <div class="block">
    ...
    export default {
        ...
        methods: {
            ...
            doposition: function (index, row) {
                ...
                this.FDataType = row.FDataType;
                this.FStartDateTime = row.FStartDateTime;
                this.position = row.sposition;
                ...
                if (FVehicleID != undefined && this.value4) {
                    ...
                    function getGather() {
                        let postData = {
                            ...
                            FAction: "QueryReportSwitchDoorCountByFVehicleGUIDs",
                            ...
                            success(data) {
                                ...
                                if (data.Result == 104) {
                                    ...
                                    $.each(Fobj.FObject.Table1, function (index, obj) {
                                        if (obj.FCount) {
                                            obj.FParkingStartDateTime = _this.parseGMT(obj.FParkingStartDateTime);
                                            obj.FParkingEndDateTime = _this.parseGMT(obj.FParkingEndDateTime);
                    ...
                    function getAll() {
                        ...
                        $.ajax({
                            ...
                            'dataType': 'json',
                            'contentType': 'application/json;charset=utf-8',
                            /*'data': '{"FTokenID":"'+_this.$store.state.FTokenID+'","FAction":"' +Faction,"FVehicleGUIDs":"' + FVehicleID,"FStartTime":"' +startTim,"FEndTime":"' +endTim,"FPageSize":"' +curpageSiz ,"FPageIndex":"' +curpageInde,"FVersion":"1.0.0"}',*/
                            'data': JSON.stringify(postData),
                            success(data) {
                                console.log("开关门数据全部", data);
                                if (data.Result == 104) {
                                    _this.$message({
                                        showClose: true,
                                        message: '_登录超时_,_五秒后跳转到登录页面_',
                                    ...
                                    $.each(Fobj.FObject.Table1, function (index, obj) {
                                        obj.FStartDateTime = _this.parseGMT(obj.FStartDateTime);
            ...
            checkDetail: function () {
                ...
                if (this.value4[0] && this.value4[1]) {
                    var t1 = this.timeChange(this.value4[0]);
                    var t2 = this.timeChange(this.value4[1]);
                    startTime = this.parseUTC(t1 + ' 00:00:00');
                    endTime = this.parseUTC(t2 + ' 23:59:59');
                }
                //请求一辆车的数据
                let postData = {
                    FTokenID: _this.$store.state.FTokenID,
                    ...
                    success: function (data) {
                        ...
                        if (data.Result == 104) {
                            ...
                            console.log("请求的单个集装箱的开关门的数据", data);
                            _this.tableData5 = [];
                            var Fobj = data;
                            var Baidu = new baidu.baiduAPI();
                            $.each(Fobj.FObject.Table1, function (index, obj) {
